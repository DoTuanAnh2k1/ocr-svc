...
def initialize_model():
    ...
    _tokenizer = AutoTokenizer.from_pretrained("5CD-AI/Vintern-1B-v3_5", trust_remote_code=True, use_fast=False)
...
def extract_image(image_file) -> str:
    global _model, _tokenizer, _device, _dtype
    
    # Initialize model if not already done
    initialize_model()
    
    pixel_values = load_image(image_file, max_num=3).to(_dtype)
    ...
    response, history = _model.chat(_tokenizer, pixel_values, question, generation_config, history=None, return_history=True)
